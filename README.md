# August target

# update all the topics here

|S No.|	Topic                         |	Start Date |	Status	| End Date    | classes|
| 1   |AWT & Swing in Java            |------------|----------|-------------| 1      |
|  1.1|Events in AWT                  |------------|----------|-------------| 1      | 
|  2  | exception handling            |------------|----------|-------------| 5      | 
|  3  |Multithreading                 |------------|----------|-------------| 6      |  
|  4  |Synchronization                |------------|----------|-------------| 5      | 
|  5  | input output and serialization|------------|----------|-------------| 6      |  
|  6  |Socket Programming & RMI       |------------|----------|-------------| 3      | 
|  7  |Collection Framework &Generics |------------|----------|-------------| 20     | 
|  8  | java features 8               |------------|----------|-------------| 6      | 
|  9  |  JDBC                         |------------|----------|-------------| 1      | 
|     |                |------------|----------|-------------| 1       | 



------------------------------------------------------------------------------------------------------------------
# July target

| S No. |	Topic                   |	Start Date |	Status	| End Date    |
| ------|-------------------------|------------|----------|-------------|
| 1	    |  OOPS's	                |03-07-2025	 |  Done	  | 03-07-2025  |
|  1.1  |CLASS,OBJ,METHOD         |03-07-2025	 |  Done	  | 03-07-2025  |
|  1.2  | CONSTRUCTORS            |03-07-2025	 |  Done	  | 03-07-2025  |
|  1.3  |Deep explanation of obj  |03-07-2025	 |  Done	  | 03-07-2025  |
|  1.4  | Relationship in class   |03-07-2025	 |  Done	  | 04-07-2025  |
|   -   | IS- A rela(INHERITANCE) |03-07-2025	 |  Done	  | 04-07-2025  |
|   -   | HAS- A rela(ASSOCIATION)|03-07-2025	 |  Done	  | 04-07-2025  |
|   -   | IS- A rela(DEPENDENCE)  |03-07-2025	 |  Done	  | 07-07-2025  |
|  1.5  | Polymorphism            |05-07-2025	 |  Done	  |07-07-2025   |
|  1.6  | Abstraction & Data hiding            |08-07-2025	 |  Done	  |08-07-2025   |
| 1.7     |Interface              |09-07-2025 |Done      |10-07-2025 |
| 1.8    |Encapsulation            |11-07-2025 -|Done      |11-07-2025 |
| 1.9    |This and Super keyword   |11-07-2025 -|  Done	  |11-07-2025 |
| 1.10   |Static variables and static blocks in Java|12-07-2025 |Done|13-07-2025 |
| 1.11   |Nested Classes           |12-07-2025 |Done       |13-07-2025 |
| 1.12   |Static Methods & Static Nested Class|12-07-2025 |Done|13-07-2025 |
| 1.13   |Packages                 |14-07-2025|running  |  15-07-2025|
| 2      |  Array                  |17-07-2025|running  |  19-07-2025|
| ------|  String                  |20-07-2025|Pending  |  22-07-2025|
| ------|-------------------------|------------|----------|-------------|
| ------|-------------------------|------------|----------|-------------|





-------------
# September target

# update all the topics here

| S No. |	Topic           |	Start Date |	Status	| End Date    |
| ------|-----------------|------------|----------|-------------|
